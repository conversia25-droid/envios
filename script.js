// =======================================================
// Configurações da Evolution API (SEM barra no final)
// =======================================================
const EVOLUTION_API_URL = "https://evoconversia.zapcompany.com.br";
const API_KEY = "429683C4C977415CAAFCCE10F7D57E11";

// Webhook padrão (oculto na UI de disparos)
...
// ===== QR TOOLBAR (aviso expiração + atualizar) =====
...
// Endpoints alternativos para pegar QR
...
// Buscar/popular instâncias — AGRUPADO e ORDENADO
...
async function fetchInstancesForDisparos() {
  ...
  const selectFallback = document.getElementById('instance-select');
  ...
  if (selectFallback) selectFallback.innerHTML = '';
    ...
    if (!openList.length) {
      ...
      openList.forEach(inst => {
        ...
        if (selectFallback) {
          ...
          selectFallback.appendChild(opt);
        }
      });
      if (hint) hint.textContent = 'Marque as instâncias que receberão os envios.';
    }
  } catch (err) {
...
// Stubs seguros (caso não existam na sua base)
...
if (typeof sendBulkMessages !== 'function') {
  window.sendBulkMessages = function () {
    ...
    if (smsg) smsg.textContent = 'Simulação de envio... (implemente sua lógica aqui)';
    if (pmsg) pmsg.textContent = 'Dica: leads no formato 55 + DDD + número, ex.: 5531988887777';
  };
}
